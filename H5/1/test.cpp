...
TEST (H5_1, 1) {
    ...
    EXPECT_EQ ("fgfg", solve ("ababfgfgfgbetabcfioeftabc")) << std::endl;
    EXPECT_EQ ("gf", solve ("gugigogqgsfdabcapbefioezf_gfgfop")) << std::endl;
    EXPECT_EQ ("aaaa", solve ("aaaaa")) << std::endl;
    EXPECT_EQ ("abcabcabc", solve ("abcabcabcabc")) << std::endl;
    ...
    EXPECT_EQ ("AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA"
               "AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAB",
        solve ("AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA"
               "AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAB"
               "AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA"
               "AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAB")) << std::endl;

    EXPECT_EQ ("abcabcabcabcabcabcabcabc",
        solve ("abcabcabcabcabcabcabcabc" "ccc" "abcabcabcabcabcabcabcabc" "kfl")) << std::endl;


    EXPECT_EQ ("aaaaccc", solve ("aaaaacccaaaacccc")) << std::endl;
}
...
TEST (H5_1, AUTO) {
    ...
    const std::size_t max_src_size = 1500;

    const std::size_t ptrn_min_size = min_src_size;
    const std::size_t ptrn_max_size = max_src_size - 1;

    const std::size_t repeat = 1;
    
    for (std::size_t irepeat = 0; irepeat < repeat; ++irepeat)
    for (std::size_t src_size = min_src_size; src_size <= max_src_size; src_size += 10) {
        for (std::size_t ptrn_size = ptrn_min_size; ptrn_size <= ptrn_max_size; ptrn_size += 10) {
